...
bool DBConnection::Connect(SQLHENV henv, const WCHAR* connectionString)
{
	...
	::wcscpy_s(stringBuffer, connectionString); // 전달받은 문자열 초기화
...
bool DBConnection::Execute(const WCHAR* query) // 전달받은 query를 실행하는 함수.
{
...
void DBConnection::Unbind() // SQLHSTMT 변수 초기화.
{
...
{
	SQLULEN size = static_cast<SQLULEN>((::wcslen(str) + 1) * 2); // WCHAR 이기때문에 *2
	*index = SQL_NTSL;
...
// ★★ BindParam과 BindCol의 실질적인 실행 함수.
// SQLHSTMT 변수를 사용한다. 그렇기때문에 반드시 SQLHSTMT 변수가 초기화 되어있어야한다.
