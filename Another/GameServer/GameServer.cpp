...
int main()
{
	...
	// Create Table
	{
		...
		DBConnection* dbConn = GDBConnectionPool->Pop(); // 커넥트 성공한 것을 꺼내온다.
		ASSERT_CRASH(dbConn->Execute(query));
		GDBConnectionPool->Push(dbConn); // 꼭 해야한다.
	}
	...
	for (int32 i = 0; i < 3; i++)
	{
		...
		ASSERT_CRASH(dbBind.Execute()); // 항상 마지막에 쿼리 실행.
