...
bool DBConnectionPool::Connect(int32 connectionCount, const WCHAR* connectionString) // 서버가 뜰때 딱 한번만.
{
	WRITE_LOCK;

	if (::SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &_environment) != SQL_SUCCESS) // SQLEENV 초기화.
		return false;
...
void DBConnectionPool::Clear()
{
	...
	if (_environment != SQL_NULL_HANDLE) // 널이 아니면
	{
