...
namespace SP // DB에 접근해서 정보를 긁어온다.
{
...
{
	ParseXmlDB(path); // Xml을 파싱한다. 구조를 분석하고 메모리에 들고있음
	//XML 만들어서 메모리에 로드하고

	GatherDBTables(); //
	GatherDBIndexes(); //
	GatherDBStoredProcedures(); // DB에 접근해서 쿼리를 날려서 정보를 긁어온다.
	//DB긁어서 정보 로드하고


	// 여기까지 실행이 되었으면 이제 xml과 db에서 데이터를 긁어왔으니 비교해야한다.
	// _xmlTables, _xmlProcedures / _dbTables, _dbProcedures

	CompareDBModel(); // 둘의 정보가 일치한가? 버전 확인 예약만 해두고
	ExecuteUpdateQueries();  // 여기서 실행한다. 업데이트 쿼리 실행.

	return true;
}

void DBSynchronizer::ParseXmlDB(const WCHAR* path) // 지난 XMLParser의 기능. 저번엔 데이터를 그냥 읽어들이기만 했지만 여기서는 저장까지 한다.
{
	XmlNode root;
	XmlParser parser;
	ASSERT_CRASH(parser.ParseFromFile(path, OUT root));

	Vector<XmlNode> tables = root.FindChildren(L"Table"); // 현존하는 모든 "Table"을 가져온다.
	for (XmlNode& table : tables)
	{
		DBModel::TableRef t = MakeShared<DBModel::Table>(); // 테이블을 담을 클래스 제작
		t->_name = table.GetStringAttr(L"name");

		Vector<XmlNode> columns = table.FindChildren(L"Column"); // 현존하는 모든 "Column"을 가져온다.
		for (XmlNode& column : columns)
		{
			DBModel::ColumnRef c = MakeShared<DBModel::Column>(); // 컬럼 담을 클래스 제작
			c->_name = column.GetStringAttr(L"name");
			c->_typeText = column.GetStringAttr(L"type"); // ★ 타입을 문자열로만 저장되어있으니 일단 저장하고
			c->_type = DBModel::Helpers::String2DataType(c->_typeText.c_str(), OUT c->_maxLength);
			// 여기서 가져온 타입 문자열을 진짜 타입으로 변환시켜준다.
...
bool DBSynchronizer::GatherDBTables()
{
	...
	while (getDBTables.Fetch()) // 데이터가 있을때까지 무한루프를 돌면서 계속 추출
	{
		...
		if (findTable == _dbTables.end()) // 중복되는 데이터가 없으면,
		{
...
void DBSynchronizer::CompareDBModel()
{
	...
	for (DBModel::TableRef& dbTable : _dbTables)
	{
		...
		if (findTable != xmlTableMap.end())
		{
			...
			CompareTables(dbTable, xmlTable); // Table 비교
...
void DBSynchronizer::CompareTables(DBModel::TableRef dbTable, DBModel::TableRef xmlTable)
{
	...
	for (DBModel::ColumnRef& dbColumn : dbTable->_columns)
	{
		...
		if (findColumn != xmlColumnMap.end())
		{
			...
			CompareColumns(dbTable, dbColumn, xmlColumn); // Colums 비교
