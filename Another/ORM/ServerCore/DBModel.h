...
// 데이터를 읽기만 하지않고, XML을 이용해서 해당 클래스들에 맞춰서 데이터를 저장하는 방법.
// 잘 보면 XML에 해당하는 어트리뷰트들이 클래스화 되어있다. DBSynchronizer::ParserXmlDB 참고.

NAMESPACE_BEGIN(DBModel) // 매크로의 현명한 사용 방법
...
enum class DataType // 랜덤 수가 아니라 SQL 서버에서 사용하는 각 타입의 실제 오브젝트 번호들
{
	None = 0,
	TinyInt = 48,
	SmallInt = 52,
...
{
public:
	String				CreateText(); // 컬럼을 사용하기 위한 query문을 생성
...
public:
	...
	bool				_identity = false; // 이 세개가
	int64				_seedValue = 0; // 아이덴티티를 가리키기위한
	int64				_incrementValue = 0; // 한 쌍이다.
	String				_default; // 기본값
