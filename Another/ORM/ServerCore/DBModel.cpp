...
String Column::CreateText() // printf같은 느낌
{
...
{
	//L"([a-z]+)([[((max|[[digit+]])]]))?
	std::wregex reg(L"([a-z]+)(\\((max|\\d+)\\))?"); // 카톡링크 regex참고, 문자열을 비교하는 c++11클래스
	// [a-z]+ a부터 z까지 1개이상 , (\\((max|\\d+)\\))? max | 숫자 1개이상, 있거나 없거나
	std::wcmatch ret; // match형은 첫번째는 전체 문자열이 나오고, 두번째부터 ()안에있는 문자열이 나온다.

	if (std::regex_match(str, OUT ret, reg) == false)
		return DataType::None;

	if (ret[3].matched) //3번째 괄호 (max|//d))? 에서 max인지 판별 비교 매치를 했는지 bool반환
		maxLen = ::_wcsicmp(ret[3].str().c_str(), L"max") == 0 ? -1 : _wtoi(ret[3].str().c_str()); // 정수로 변환
		//같으면 0 반환
	else
		maxLen = 0;
